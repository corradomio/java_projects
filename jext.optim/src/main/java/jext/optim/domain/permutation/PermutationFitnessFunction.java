...
public class PermutationFitnessFunction implements FitnessFunction<Permutation> {
    ...
    public PermutationFitnessFunction(float[][] distances) {
        this.distances = distances;
    }

    @Override
    public float fitness(Permutation permutation) {
        int[] perm = permutation.permutation();
        int n = perm.length;
        float distance = distances[perm[n-1]][perm[0]];
        for (int i = 1; i < n; i++)
