...
public class ContextTypeSolver extends CompositeTypeSolver {
    ...
    public ContextTypeSolver addImport(ImportDeclaration n) {
        ...
        if (n.isAsterisk())
            this.starImports.add(this.starImports.size(), JavaUtils.namespaceOf(importName));
        if (!n.isStatic())
    ...
    public ResolvedMethodDeclaration resolve(MethodCallExpr mce) {
        try {
        ...
        catch (UnsolvedSymbolException | UnsupportedOperationException | MethodAmbiguityException | IllegalArgumentException e) {
            ;
        }
        catch (RuntimeException e) {
            String message = e.getMessage();
            if (message == null)
                logger.errorf("[%s] %s: %s - %s", e.getClass().toString(), mce.toString(), e.getMessage(), filename);
            ...
            logger.errorf("[%s] %s: %s - %s", t.getClass().toString(), mce.toString(), t.getMessage(), filename);
        }
