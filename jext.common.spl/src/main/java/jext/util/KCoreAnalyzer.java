...
public class KCoreAnalyzer {
    ...
    public static List<java.util.Map<String, Object>> computeKCore (List<java.util.Map<String, Object>> nodes){
        for(java.util.Map<String, Object> node: nodes){
            ...
            int degree = new java.util.ArrayList<>((Collection<Integer>)node.get("neighbors")).size();
            ...
            do{
                ...
                for(java.util.Map<String, Object> node: nodes){
                    if(Boolean.parseBoolean(node.get("justDeleted").toString())){
                        java.util.ArrayList<Integer> neighbors =(ArrayList<Integer>) ((Collection<Long>)node.get("neighbors")).stream().mapToInt(r -> r.intValue()).boxed().collect(Collectors.toList());
                        for(int x: neighbors){
