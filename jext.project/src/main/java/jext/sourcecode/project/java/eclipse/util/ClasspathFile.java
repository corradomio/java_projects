package jext.sourcecode.project.java.eclipse.util;
...
public class ClasspathFile {
    ...
    public ClasspathFile(File classpath) {
        ...
        this.moduleHome = classpath.getParentFile();
        ...
        XPathUtils.selectElements(elt, "classpathentry[@kind='src']")
                .forEach(esrc -> {
                    ...
                    if (!path.isEmpty() && combineaccessrules.isEmpty())
                        dirs.add(new File(moduleHome, path));
                });
        return dirs;
    }
    ...
    public List<File> getLocalLibraries() {
        ...
        XPathUtils.selectElements(elt, "classpathentry[@kind='lib']")
                .forEach(esrc -> {
                    String path = XPathUtils.getValue(esrc, "@path", "");
                    if (!path.isEmpty())
                        localLibs.add(FileUtils.toFile(moduleHome, path));
                });
        return localLibs;
    }
}
